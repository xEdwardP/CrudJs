...
export default function App() {
  ...
  function openEdit(p: Product) { setSelected(p); setForm({ name: p.name, description: p.description || "", price: String(p.price) }); setError(""); setModal("edit"); }
  ...
  async function handleCreate() {
    ...
    const res = await fetch(API, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, price: Number(form.price) }) });
    if (res.ok) {
      fetchProducts();
      closeModal();
  ...
  async function handleEdit() {
    if (!form.name || form.price === "") { setError("Nombre y precio son requeridos."); return; }
    const res = await fetch(`${API}/${selected!.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, price: Number(form.price) }) });
    if (res.ok) {
      fetchProducts();
      closeModal();
  ...
  return (
    <div className="min-h-screen bg-slate-50 px-6 py-10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Crud de productos</h1>
          <p className="text-slate-400 mt-1 text-sm">Negocios Web - 2026</p>
        </div>
            ...
            <div className="overflow-x-auto rounded-xl border border-slate-200">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="text-center py-10 text-slate-400">No se encontraron productos.</td>
                    </tr>
                  ) : filtered.map(p => (
            ...
            {modal === "view" && selected && (
              <>
                ...
                <div className="space-y-3 text-sm">
                  {[["ID", `#${selected.id}`], ["Nombre", selected.name], ["Descripción", selected.description || "—"], ["Precio", `L ${Number(selected.price).toFixed(2)}`]].map(([label, val]) => (
            ...
            {(modal === "create" || modal === "edit") && (
              <>
                ...
                <div className="space-y-4">
                  {[{ label: "Nombre *", key: "name", type: "text" }, { label: "Precio *", key: "price", type: "number" }].map(({ label, key, type }) => (
                    <div key={key}>
